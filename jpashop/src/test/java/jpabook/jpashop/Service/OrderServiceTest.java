...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        //given
        Member member = createMember("hwang");
        ...
        //then
        Order getOrder = orderRespository.findOne(orderId);

        Assert.assertEquals("상품 주문시 상태는 ORDER", OrderStatus.ORDER,getOrder.getStatus());
        Assert.assertEquals("주문한 상품 종류 수가 정확해야한다",1,getOrder.getOrderItems().size());
        Assert.assertEquals("주문 가격은 가격 * 수량인다", 10000*count, getOrder.getTotalPrice());
        Assert.assertEquals("주문 수량만큼 재고가 줄어야 한다.",10-count,item.getStockQuantity());
    }
    ...
    public void 주문취소() throws Exception {
        //given
        Member member=createMember("asd");
        ...
        Assert.assertEquals("주문이 취소된 상품은 그만큼 증가해야한다.",10,item.getStockQuantity());
